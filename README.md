...
**Telegram 通知**：对比 `telegram` 服务每次刷新前后的快照，向 Bot 推送「新国家上架」或「补货」（库存 0→有货，以及已有货时库存增加，例如 5→20）。补货对每次库存增加都会推送，默认无 6 小时静音（`TELEGRAM_ALERT_RESTOCK_COOLDOWN_MS` 仅作可选短冷却防抖动）。向 Bot 发一条私聊消息后，服务会自动从 `getUpdates` 发现 chat id 并写入数据库；也可手动填入 `TELEGRAM_NOTIFY_CHAT_ID`。**勿将 Bot Token 写入 Git**；若 Token 曾在聊天中泄露，请在 @BotFather 重置。
